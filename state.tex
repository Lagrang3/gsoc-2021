...
\begin{example}
   ...
   field which is a sub-class), and let $g$ be a primite root of $p$, ie. in the
   sense that the least value of $k$ for which $g^k \equiv 1 \pmod p$ is
...
innumerable representations of real numbers and complex numbers.
...
number representation types: \verb|float| and \verb|double|.
...
\cite{springel_2020,adamek_2016,pippig_13}, \fftw{}s approach to $D$-dimensional
\dft\ consists in the
decomposition of problem domain along one dimension, having as a consequence that
