...
interface for \fft\ within \boostmath\ library. The implementation will consist
...
and a back-end to the state of the art \fftw\ library for the special cases of
...
A possible user interface will consist of two overloads of the abstract \dft\
routine: 
Listings \ref{ls:dft1} and \ref{ls:dft2}. The size of the \dft\ is deduced 
...
\noindent Two other overloads of the abstract complex \dft\ are needed to release the user
from the burden of computing the root of unity by himself:
Listings \ref{ls:complex_dft1} and \ref{ls:complex_dft2}. The size of the \dft\
is again deduced as $n=$\texttt{std::distance(beg1,end1)} and \texttt{dft\_type}
...
void
complex_dft(InputIterator beg1, InputIterator end1, 
            dft_type t);
\end{lstlisting}
...
\noindent Finally there will be another routine to compute convolutions,
...
\begin{itemize}
    \item template routines for computing \dft\ using different algorithms,
    ...
    \item a back-end for some \fftw\ routines,
    \item a simple and flexible user interface that hides implementation details,
